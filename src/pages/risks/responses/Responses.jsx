...
const STATUS_CONFIG = {
  DRAFT:       { label: "Draft",        icon: CircleDashed,  pill: "bg-slate-100 text-slate-600 border-slate-200" },
...
function getType(type)     { return RESPONSE_TYPE_CONFIG[type] || { label: type || "—", icon: ShieldCheck, bg: "bg-slate-50", text: "text-slate-500", border: "border-slate-200", dot: "bg-slate-300" } }
function getStatus(status) { return STATUS_CONFIG[status]      || { label: "No status", icon: CircleDashed, pill: "bg-slate-50 text-slate-400 border-slate-100" } }

function shortId(id)    { return id?.slice(0, 8)?.toUpperCase() }
function fmtDate(val)   { return val ? new Date(val).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) : null }
...
const navigate = useNavigate()
  ...
  const dates = [
    ...
    { label: "Operating check",  value: fmtDate(response.date_monitored_operating) },
  ].filter((d) => d.value)
  ...
  return (
    ...
    >
      {/* Left accent */}
      <div className={`absolute left-0 top-4 bottom-4 w-[3px] rounded-r-full ${type.dot}`} />
      ...
      <div className="flex items-start justify-between gap-3 pl-1">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${status.pill}`}>
      ...
      <p className="text-sm text-slate-700 leading-relaxed line-clamp-2 pl-1">
        {response.response_description || <span className="italic text-slate-400">No description provided.</span>}
      </p>
      ...
      {response.evidence_notes && (
        <div className="flex items-start gap-2 bg-slate-50 border border-slate-100 rounded-lg px-3 py-2">
          ...
          <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed">{response.evidence_notes}</p>
        </div>
      )}
      ...
      {dates.length > 0 ? (
        <div className="flex flex-wrap gap-3">
          {dates.map((d) => (
            <div key={d.label} className="flex items-center gap-1 text-xs text-slate-500">
      ...
      <div className="flex items-center justify-between pt-2 border-t border-slate-100">
        <div className="flex items-center gap-1.5 text-xs text-slate-400">
          ...
          <span className="font-mono">{shortId(response.responsible_employee)}</span>
        </div>
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Responses</h1>
            <p className="text-sm text-slate-500 mt-0.5">
              Control measures and actions linked to this risk
            </p>
          </div>
          <button className="flex items-center gap-2 px-4 py-2 bg-[#3B1F6A] text-white text-sm font-medium rounded-lg hover:bg-[#2e1854] transition-colors shadow-sm shrink-0">
