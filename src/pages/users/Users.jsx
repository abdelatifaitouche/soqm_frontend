...
import { createUser, updateUser, deleteUser } from "@/api/endpoints/usersApi"
...
const EMPTY_FORM = { first_name: "", last_name: "", email: "", password: "", is_active: true }
...
export default function UsersPage() {
  ...
  const openEdit = (user, e) => {
    e.stopPropagation()
    setSelected(user)
    setForm({ first_name: user.first_name, last_name: user.last_name, email: user.email, password: "", is_active: user.is_active })
    setFormError("")
    setFormOpen(true)
  }
            ...
            <div className="space-y-1.5">
              ...
              <button type="button" onClick={() => setForm((f) => ({ ...f, is_active: !f.is_active }))}
                className={`w-full h-10 rounded-lg border text-sm font-medium transition-colors ${
                  form.is_active ? "bg-emerald-50 border-emerald-200 text-emerald-700" : "bg-gray-50 border-gray-200 text-gray-500"
                }`}>
