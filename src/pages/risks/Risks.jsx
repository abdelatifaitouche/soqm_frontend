...
// value 1 = low (1 green segment), 2 = med (2 amber), 3 = high (3 red)
const LEVEL = {
...
function Pagination({ page, totalPages, total, size, onPageChange, loading }) {
      ...
      <div className="flex items-center gap-1">
        ...
        {pages.map((p, i) => p === "…"
          ...
          : <button key={p} onClick={() => onPageChange(p)} disabled={loading}
              className={`flex size-7 items-center justify-center rounded-md text-xs font-medium transition-colors ${p === page ? "bg-[#3B1F6A] text-white" : "border border-border bg-card text-muted-foreground hover:text-foreground hover:bg-muted"}`}>
              {p}
            </button>
        )}
...
function Risks() {
  ...
  // Stable filters object — only non-empty values
